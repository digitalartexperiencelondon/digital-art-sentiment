#define parse256RGB(r,g,b)  vec3(float(r)/255.0,float(g)/255.0,float(b)/255.0)
...
#define COLOR_NEUTRAL     parse256RGB(255, 255, 255)  // GREYSCALE/WHITE (the same as our 'start' screen)
...
Sentiment getSentiment(float data[8], int idx);
...
void main() {
    ...
    vec3 scene = COLOR_NEUTRAL;
    for(int idx = 0; idx < 8; idx++) {
        Sentiment sentiment = getSentiment(u_audio, idx);
        vec2 center = vec2(sentiment.amount);
        center.x = smoothstep(-1.0, 1.0, sin(sentiment.amount * time));
        float radius = 0.5*smoothstep(-1.0, 1.0, cos(sentiment.amount*sentiment.amount * time));
